...
maxvalue <- as.integer(isteps[mxind]/ndays)
...
hist(d2steps-dsteps, breaks = 12, xlab = "Steps", ylab = "Number of Days", main = "Differential Steps per Day, With and Without Missing Values Imputed")
...
```
An now I create three plots. These is the weekend plot, the weekday plot, and a differential plot (weekday minus weekend). I'll discuss the interpretation of these plots at the conclusion of the document. Note that I use the base plotting system, not lattice. I use the parameter mfrow to create a panel of three plots, where I add at differential plot which is in response to the assignment's requests to make a comparison between weekend and weekday activity.

```{r weekdayplots, echo = TRUE, , fig.height=12, fig.width=8}
par(mfrow = c(3,1))
...
There is really not too much of a surprise here. Of course the x-axis that I have created for these plots looks more normal, having real time plotted along that axis. 

The third graph plots the difference in interval averages for weekdays and weekends. There is a lot of noise due to the small interval size that it makes it heard to draw much of a conclusion, and this is true for all three plots. You can see that there are more consistently steps taken early in the morning for workdays, and more steps taken in the afternoon on weekend days, though they tend to be erratic.
